...
def display_fullimages(origimage, gtimage, segimage, crfsegimage ):
    ...
    plt.title('1st pred')
    plt.subplot(235) 
    ret, errorImage = cv2.threshold(cv2.absdiff(segimage,gtimage),0.5, 255, cv2.THRESH_BINARY)
...
for n in range(maxn):
    ...
    origimage, gtimage, segimage, crfsegimage = load_images(origimagefile, gtimagefile,  segimagefile, crfsegimagefile)
    ...
    for label in range(1, len(labels)):
        print('%10.10s \t %3.2f \t %d \t %d \t %d' % (labels[label], hist_pred[label][0]/num_fg_pixels, hist_gt[label][0], hist_pred[label][0], hist_crf[label][0]))
