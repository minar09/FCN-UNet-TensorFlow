    ...
    # test-mode
        ...
        for itr1 in range(validation_dataset_reader.get_num_of_records()//2):
            ...
            for itr2 in range(FLAGS.batch_size):
                ...
                #crfimage, crfoutput = fd.crf(valid_images[itr2].astype(np.uint8),logits[itr2])
                ...
                crossMat = fd._calcCrossMat(valid_annotations[itr2].astype(np.uint8), pred[itr2].astype(np.uint8),
                                         NUM_OF_CLASSESS)
